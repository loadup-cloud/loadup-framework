...
- [ ] 决策项拍板（DESIGN.md 第 7 节）：许可证、Cache facade、Authorization 后端、RetryTask 路线、ORM
...
- [ ] **retrytask 修复**：统一主键 `VARCHAR(64)` UUID、线程池配置接线、未配置 bizType 兜底扫描、README/代码同步；输出 JobRunr binder 可行性评估
...
- [ ] 运行 `/docs-sync` 同步文档站
- [ ] 修复各组件 README 与代码脱节问题（retrytask、gotone、scheduler 许可证标识等）
...
- [ ] 可选 binder 落地：cache-binder-jetcache（多级 + 异步刷新）、retrytask-engine-jobrunr、authorization-binder-spring-security
