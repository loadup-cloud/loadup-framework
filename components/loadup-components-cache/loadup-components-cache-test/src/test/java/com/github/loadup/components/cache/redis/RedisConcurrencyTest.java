...
public class RedisConcurrencyTest extends BaseCacheTest {
    ...
    void testConcurrentReadWriteMix() throws InterruptedException {
        ...
        try {
            ...
            log.info("Concurrent operations completed - Writes: {}, Reads: {}, Deletes: {}",
    ...
    void testConcurrentUpdateSameKey() throws InterruptedException {
        ...
        try {
            ...
            assertTrue(updatedNames.contains(finalUser.getName()),
                "Final value should be from one of the updates");

            log.info("Concurrent updates completed - Total unique updates: {}, Final value: {}",
                updatedNames.size(), finalUser.getName());
        } finally {
            executor.shutdown();
        }
    }
    ...
    void testDataConsistencyUnderHighConcurrency() throws InterruptedException {
        ...
        try {
            ...
            assertEquals(allKeys.size(), validCount,
                "All written keys should be retrievable with correct data");

            log.info("Data consistency test completed - Total keys: {}, Valid: {}",
                allKeys.size(), validCount);
        } finally {
            executor.shutdown();
        }
    }
