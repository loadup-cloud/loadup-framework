package com.github.loadup.components.scheduler.starter.initializer;
...
public class SchedulerJobInitializer implements SmartInitializingSingleton {
  ...
  public void afterSingletonsInstantiated() {
    ...
    for (String beanName : beanNames) {
      ...
      Map<Method, DistributedScheduler> annotatedMethods =
          MethodIntrospector.selectMethods(
              targetClass,
              (MethodIntrospector.MetadataLookup<DistributedScheduler>)
                  method ->
                      AnnotatedElementUtils.findMergedAnnotation(
                          method, DistributedScheduler.class));
  ...
  private void registerJob(Object bean, Method method, DistributedScheduler ann) {
    ...
    if (binding == null) {
      log.error(
          "Failed to register task [{}]: No binding config found for tag [{}]", taskName, bizTag);
      return;
    }
