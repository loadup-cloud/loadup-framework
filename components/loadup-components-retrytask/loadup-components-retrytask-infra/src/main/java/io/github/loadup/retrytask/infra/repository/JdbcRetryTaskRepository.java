...
public class JdbcRetryTaskRepository implements RetryTaskRepository {
    ...
    private void initSqlMap() {
        ...
        // FIND_TASKS_TO_RETRY - 使用 priority DESC 数值排序
        if (DbType.ORACLE == dbType) {
            sqlMap.put(
                    "FIND_TASKS_TO_RETRY",
                    String.format(
                            "SELECT * FROM %s WHERE next_retry_time < ? AND status = ? ORDER BY priority DESC, next_retry_time ASC OFFSET 0 ROWS FETCH NEXT ? ROWS ONLY",
                            tableName));
        } else {
            // MySQL, PgSQL
            sqlMap.put(
                    "FIND_TASKS_TO_RETRY",
                    String.format(
                            "SELECT * FROM %s WHERE next_retry_time < ? AND status = ? ORDER BY priority DESC, next_retry_time ASC LIMIT ?",
                            tableName));
        }

        // FIND_TASKS_TO_RETRY_BY_BIZ_TYPE - 使用 priority DESC 数值排序
        if (DbType.ORACLE == dbType) {
            sqlMap.put(
                    "FIND_TASKS_TO_RETRY_BY_BIZ_TYPE",
                    String.format(
                            "SELECT * FROM %s WHERE biz_type = ? AND next_retry_time < ? AND status = ? ORDER BY priority DESC, next_retry_time ASC OFFSET 0 ROWS FETCH NEXT ? ROWS ONLY",
                            tableName));
        } else {
            // MySQL, PgSQL
            sqlMap.put(
                    "FIND_TASKS_TO_RETRY_BY_BIZ_TYPE",
                    String.format(
                            "SELECT * FROM %s WHERE biz_type = ? AND next_retry_time < ? AND status = ? ORDER BY priority DESC, next_retry_time ASC LIMIT ?",
                            tableName));
        }
    ...
    public List<RetryTask> findTasksToRetry(LocalDateTime nextRetryTime, int limit) {
        ...
        List<RetryTaskDO> list =
                jdbcTemplate.query(sql, rowMapper, nextRetryTime, RetryTaskStatus.PENDING.name(), limit);
    ...
    public List<RetryTask> findTasksToRetryByBizType(String bizType, LocalDateTime nextRetryTime, int limit) {
        ...
        List<RetryTaskDO> list =
                jdbcTemplate.query(sql, rowMapper, bizType, nextRetryTime, RetryTaskStatus.PENDING.name(), limit);
        return list.stream().map(RetryTaskDOConverter::toDomain).collect(Collectors.toList());
    }
