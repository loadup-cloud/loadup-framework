...
@TestPropertySource(
    properties = {
      ...
      "loadup.testcontainers.elasticsearch.enabled=true"
    })
class SharedElasticsearchContainerIT extends AbstractElasticsearchContainerTest {
  ...
  void testContainerProperties() {
    assertNotNull(
        SharedElasticsearchContainer.getHttpHostAddress(), "HTTP host address should not be null");
  ...
  void testIndexDocument() throws Exception {
    ...
    try (RestClient restClient = RestClient.builder(new HttpHost(host, port, "http")).build()) {
      ...
      IndexResponse indexResponse = client.index(i -> i.index(indexName).id(docId).document(doc));
      ...
      // Delete the document
      DeleteResponse deleteResponse = client.delete(d -> d.index(indexName).id(docId));
  ...
  void testSearchDocument() throws Exception {
    ...
    try (RestClient restClient = RestClient.builder(new HttpHost(host, port, "http")).build()) {
      ...
      SearchResponse<TestDocument> searchResponse =
          client.search(
              s -> s.index(indexName).query(q -> q.match(m -> m.field("name").query("Bob"))),
              TestDocument.class);
