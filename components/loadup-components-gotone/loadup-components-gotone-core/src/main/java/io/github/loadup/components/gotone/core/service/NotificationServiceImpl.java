...
public class NotificationServiceImpl implements NotificationService {
    ...
    public NotificationResponse send(NotificationRequest request) {
        log.info(">>> [GOTONE] 开始发送通知，serviceCode={}, receivers={}", request.getServiceCode(), request.getReceivers());

        // 1. 幂等性检查
        if (request.getRequestId() != null && recordRepository.existsByRequestId(request.getRequestId())) {
        ...
        // 3. 查询启用的渠道配置
        List<ServiceChannelDO> channelConfigs =
                channelRepository.findEnabledChannelsByServiceCode(request.getServiceCode());
        ...
        for (ServiceChannelDO channelConfig : channelConfigs) {
            try {
                ...
                log.error(">>> [GOTONE] 渠道发送失败，channel={}, error={}", channelConfig.getChannel(), e.getMessage(), e);
    ...
    public void sendAsync(NotificationRequest request) {
        try {
            ...
            log.error(">>> [GOTONE] 异步发送失败，serviceCode={}, error={}", request.getServiceCode(), e.getMessage(), e);
        }
    }
    ...
    private ChannelSendResult sendToChannel(
            NotificationRequest request, ServiceChannelDO channelConfig, String traceId) {

        String channel = channelConfig.getChannel();
        log.info(">>> [GOTONE] 发送到渠道，channel={}, provider={}", channel, channelConfig.getProvider());

        // 1. 渲染模板
        String content = templateProcessor.render(channelConfig.getTemplateContent(), request.getTemplateParams());

        // 2. 获取渠道提供商
        NotificationChannelProvider provider =
                channelManager.getProvider(NotificationChannel.valueOf(channel), channelConfig.getProvider());

        if (provider == null) {
            log.error(">>> [GOTONE] 渠道提供商不可用，channel={}, provider={}", channel, channelConfig.getProvider());
        ...
        if (validReceivers.isEmpty()) {
            log.warn(">>> [GOTONE] 无有效收件人，channel={}, receivers={}", channel, request.getReceivers());
        ...
        try {
            ...
            log.error(">>> [GOTONE] 渠道发送异常，channel={}, error={}", channel, e.getMessage(), e);
    ...
    private Map<String, Object> buildChannelData(
            ServiceChannelDO channelConfig, NotificationRequest request, String receiver) {
    ...
    private List<String> filterReceivers(List<String> receivers, String channel) {
        return switch (NotificationChannel.valueOf(channel)) {
            case EMAIL -> receivers.stream().filter(this::isValidEmail).toList();
            case SMS -> receivers.stream().filter(this::isValidPhone).toList();
            case PUSH ->
                receivers.stream().filter(this::isPushCompatibleReceiver).toList();
            default -> receivers;
        };
    }
