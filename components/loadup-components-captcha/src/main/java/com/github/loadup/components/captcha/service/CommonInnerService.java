...
import com.github.loadup.capability.common.enums.Base64HeaderEnum;
...
import com.github.loadup.components.captcha.properties.CaptchaProperties;
...
public class CommonInnerService {
    ...
    protected void saveCache(String key, String value) {
        ...
        CaptchaFactory.CACHE_MAP.put(key, value);
    }

    protected String generateChinese() {
        char[] cs = new char[captchaProperties.getLength()];
     ...
     * 返回随机汉字
     */
    private char alphaHan() {
        return DELTA.charAt(RandomUtils.nextInt(0, DELTA.length()));
    }
    ...
    public String toBase64(ByteArrayOutputStream outputStream) {
        return toBase64(Base64HeaderEnum.PNG.getCode(), outputStream);
    }

    public String toBase64(String type, ByteArrayOutputStream outputStream) {
    ...
    public void drawLine(int num, Color color, Graphics2D g) {
        for (int i = 0; i < num; i++) {
            ...
            int x1 = RandomUtils.nextInt(0, captchaProperties.getWidth() - 10);
            int y1 = RandomUtils.nextInt(5, captchaProperties.getHeight() - 5);
            int x2 = RandomUtils.nextInt(10, captchaProperties.getWidth() + 10);
            int y2 = RandomUtils.nextInt(2, captchaProperties.getHeight() - 2);
    ...
    public Font getChineseFont() {
        return new Font("楷体", Font.PLAIN, 28);
    }

    public Font getFont() {
        try {
            return getFont(CharFontEnum.ACTIONJ);
        } catch (Exception e) {
            return new Font("Arial", Font.BOLD, 32);
        }
    }

    public Font getFont(CharFontEnum font) throws IOException, FontFormatException {
        return getFont(font, 32f);
    }

    public Font getFont(CharFontEnum font, float size) throws IOException, FontFormatException {
        return getFont(font, Font.BOLD, size);
    }
    ...
    public boolean validate(String key, String userInputValue) {
        String cacheValue = CaptchaFactory.CACHE_MAP.get(key);
        return StringUtils.equalsIgnoreCase(cacheValue, userInputValue);
    }
}