...
public class RandomUtil {
   ...
   *
   * <p>注意：此方法返回的{@link ThreadLocalRandom}不可以在多线程环境下共享对象，否则有重复随机数问题。 见：<a
   * href="https://www.jianshu.com/p/89dfe990295c">https://www.jianshu.com/p/89dfe990295c</a>
   *
   * @since 3.1.2
   */
   ...
   * see: <a
   * href="http://ifeve.com/jvm-random-and-entropy-source/">http://ifeve.com/jvm-random-and-entropy-source/</a>
   *
   * <p>相关说明见：<a
   * href="https://stackoverflow.com/questions/137212/how-to-solve-slow-java-securerandom">how-to-solve-slow-java-securerandom</a>
   *
  ...
  /** 获得指定范围内的随机数 */
  public static int randomInt(
      int min, int max, final boolean includeMin, final boolean includeMax) {
    if (!includeMin) {
      min++;
    }
    if (includeMax) {
  ...
  /** 获得指定范围内的随机数 */
  public static long randomLong(
      long min, long max, final boolean includeMin, final boolean includeMax) {
    if (!includeMin) {
      min++;
    }
    if (includeMax) {
   ...
   */
  public static double randomDouble(
      final double limit, final int scale, final RoundingMode roundingMode) {
   ...
   */
  public static BigDecimal randomBigDecimal(
      final BigDecimal minInclude, final BigDecimal maxExclude) {
    return NumberUtil.toBigDecimal(
        getRandom().nextDouble(minInclude.doubleValue(), maxExclude.doubleValue()));
  }
