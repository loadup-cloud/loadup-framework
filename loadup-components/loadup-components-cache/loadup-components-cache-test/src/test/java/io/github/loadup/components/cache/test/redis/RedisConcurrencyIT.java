...
public class RedisConcurrencyIT extends BaseRedisCacheTest {
    ...
    void testConcurrentWrites() throws InterruptedException {
        ...
        try (ExecutorService executor = Executors.newFixedThreadPool(threadCount)) {
                ...
                executor.submit(() -> {
                    try {
                        for (int j = 0; j < operationsPerThread; j++) {
                            ...
                            boolean result = redisBinding.set(key, user);
                            if (result) {
            ...
            assertEquals(threadCount * operationsPerThread, successCount.get(), "All write operations should succeed");
        }
    }
    ...
    void testConcurrentReads() throws InterruptedException {
        ...
        try (ExecutorService executor = Executors.newFixedThreadPool(threadCount)) {
            ...
            // Then
            assertTrue(completed, "All threads should complete");
            assertEquals(threadCount * readsPerThread, successCount.get(), "All read operations should succeed");
        }
    }
    ...
    void testConcurrentReadWriteMix() throws InterruptedException {
        ...
        try (ExecutorService executor = Executors.newFixedThreadPool(threadCount)) {
                        ...
                        for (int j = 0; j < operationsPerThread; j++) {
                            ...
                            switch (operation) {
                                case 0: // Write
                                    ...
                                    break;
                                case 1: // Read
                                    redisBinding.getObject(key, User.class);
            ...
            // Then
            assertTrue(completed, "All threads should complete");
            int totalOperations = statistics.get("writes").get()
                    + statistics.get("reads").get()
                    + statistics.get("deletes").get();
            assertEquals(threadCount * operationsPerThread, totalOperations, "All operations should be executed");
    ...
    void testConcurrentUpdateSameKey() throws InterruptedException {
        ...
        try (ExecutorService executor = Executors.newFixedThreadPool(threadCount)) {
            ...
            User finalUser = redisBinding.getObject(key, User.class);
            assertNotNull(finalUser, "Final user should exist");
            assertTrue(updatedNames.contains(finalUser.getName()), "Final value should be from one of the updates");
        ...
        try {
            ...
            for (int i = 0; i < threadCount; i++) {
                ...
                executor.submit(() -> {
                    try {
                        for (int j = 0; j < operationsPerThread; j++) {
                            final String key = keyPrefix + threadIndex + ":" + j; // Make 'key' effectively final
                            final User user =
                                    User.createTestUser(threadIndex + "-" + j); // Make 'user' effectively final
                            final String name = "Thread-" + threadIndex + "-Op-" + j; // Make 'name' effectively final
                            user.setName(name);
                            redisBinding.set(key, user); // Ensure all variables are final
                        }
                    } finally {
                        latch.countDown();
                    }
            ...
            assertEquals(allKeys.size(), validCount, "All written keys should be retrievable with correct data");

            log.info("Data consistency test completed - Total keys: {}, Valid: {}", allKeys.size(), validCount);
        } finally {
            executor.shutdown();
        }
    }
