...
public class HuaweiSmsProvider implements NotificationChannelProvider {
    ...
    public ChannelSendResponse send(ChannelSendRequest request) {
        log.info(">>> [GOTONE-SMS-HUAWEI] 开始发送短信, receivers={}, content length={}",
        ...
        try {
            ...
            for (String phoneNumber : request.getReceivers()) {
                ...
                // 模拟调用华为云API
                boolean sendSuccess = mockHuaweiSmsApi(phoneNumber, currentSender, currentSignature, templateId, request);
    ...
    public boolean isAvailable() {
        boolean available = appKey != null && !appKey.trim().isEmpty()
    ...
    private boolean mockHuaweiSmsApi(String phoneNumber, String sender, String signature,
            String templateId, ChannelSendRequest request) {
        // TODO: 实际集成时替换为真实的华为云SDK调用
        log.debug(">>> [GOTONE-SMS-HUAWEI] Mock API Call: phone={}, sender={}, signature={}, template={}",
                maskPhoneNumber(phoneNumber), sender, signature, templateId);
