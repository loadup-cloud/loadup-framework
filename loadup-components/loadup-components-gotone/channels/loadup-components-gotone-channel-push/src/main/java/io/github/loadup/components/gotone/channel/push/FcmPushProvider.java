...
public class FcmPushProvider implements NotificationChannelProvider {
    ...
    public ChannelSendResponse send(ChannelSendRequest request) {
        log.info(">>> [GOTONE-PUSH-FCM] 开始发送推送, receivers={}, content length={}",
    ...
    private boolean mockFcmApi(String deviceToken, String title, String body,
            String sound, Integer badge, Map<String, Object> extras) {
        // TODO: 实际集成时替换为真实的 FCM SDK 调用
        log.debug(">>> [GOTONE-PUSH-FCM] Mock API Call: token={}, title={}, body length={}",
                maskDeviceToken(deviceToken), title, body.length());
