...
public class NotificationChannelManager {
    ...
    public NotificationChannelManager(List<NotificationChannelProvider> providers) {
        if (providers != null) {
            for (NotificationChannelProvider provider : providers) {
                ...
                log.info(">>> [GOTONE] Registered provider: {} for channel: {}",
                        providerName, channel);
            }
        }
        log.info(">>> [GOTONE] NotificationChannelManager initialized with {} channels",
            providerMap.size());
    }
    ...
    public NotificationChannelProvider getProvider(NotificationChannel channel, String providerName) {
        ...
        if (provider == null) {
            log.warn(">>> [GOTONE] Provider {} not found for channel {}, available: {}",
                providerName, channel, providers.keySet());
    ...
    public boolean isChannelSupported(NotificationChannel channel) {
        ...
        return providers != null &&
            providers.values().stream().anyMatch(NotificationChannelProvider::isAvailable);
    }
}
