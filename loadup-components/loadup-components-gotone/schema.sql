...
(
  ...
  COLLATE = utf8mb4_unicode_ci COMMENT ='通知服务配置表';
...
(
  ...
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci COMMENT ='服务渠道映射表';
...
(
  ...
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci COMMENT ='通知发送记录表';
...
INSERT INTO gotone_service_channel (id, service_code, channel, template_code, template_content, channel_config,
                                    provider, send_strategy, retry_config, enabled, priority, created_at)
VALUES ('CHANNEL_001', 'ORDER_CREATED', 'EMAIL', 'ORDER_CREATED_EMAIL',
        '尊敬的${userName}，您的订单${orderNo}已创建成功。', '{"subject":"订单创建通知","from":"order@loadup.com"}',
        'smtp', 'ASYNC', '{"maxRetries":3,"retryInterval":60}', 1, 10, NOW()),
       ('CHANNEL_002', 'ORDER_CREATED', 'SMS', 'ORDER_CREATED_SMS', '您的订单${orderNo}已创建，金额${amount}元。',
        '{"signName":"LoadUp商城","templateId":"SMS_123456"}', 'aliyun', 'ASYNC', '{"maxRetries":3,"retryInterval":60}',
        1, 9, NOW()),
       ('CHANNEL_003', 'PAYMENT_SUCCESS', 'PUSH', 'PAYMENT_SUCCESS_PUSH', '支付成功！订单${orderNo}，金额${amount}元。',
        '{"title":"支付成功","sound":"default"}', 'fcm', 'ASYNC', '{"maxRetries":2,"retryInterval":30}', 1, 10, NOW()),
       ('CHANNEL_004', 'VERIFICATION_CODE', 'SMS', 'VERIFICATION_CODE_SMS', '您的验证码是${code}，${minutes}分钟内有效。',
        '{"signName":"LoadUp","templateId":"SMS_VERIFY"}', 'aliyun', 'SYNC', '{"maxRetries":1,"retryInterval":10}', 1,
        10, NOW());
