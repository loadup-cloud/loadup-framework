...
所有具体实现（Local / Nacos / Apollo）继承 `AbstractConfigCenterBinder`，后者继承框架 `AbstractBinder`，在
`afterConfigInjected()` 钩子中完成 SDK 初始化。
...
`BindingManagerSupport` 在首次调用 `getBinding()` 时懒加载 `BindingMetadata`，按 `binderType` 匹配，注入配置并初始化对应
Binder 实例。
