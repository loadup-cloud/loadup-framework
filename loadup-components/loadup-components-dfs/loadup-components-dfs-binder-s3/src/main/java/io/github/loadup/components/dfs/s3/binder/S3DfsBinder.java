...
public class S3DfsBinder extends AbstractDfsBinder<S3DfsBinderCfg, DfsBindingCfg>
    ...
    protected void afterConfigInjected(String name, S3DfsBinderCfg binderCfg, DfsBindingCfg bindingCfg) {
        ...
        try {
            ...
            if (this.binderCfg.getEndpoint() != null
                    && !this.binderCfg.getEndpoint().isEmpty()) {
    ...
    public FileMetadata upload(FileUploadRequest request) {
        try {
            ...
            if (request.getMetadata() != null) {
                request.getMetadata().forEach((key, value) -> s3Metadata.put(META_CUSTOM_PREFIX + key, value));
            }
            ...
            log.info("File uploaded successfully to S3: {} -> {}, metadata saved", request.getFilename(), objectKey);
    ...
    public boolean delete(String fileId) {
        try {
            ...
            DeleteObjectRequest deleteMetaRequest = DeleteObjectRequest.builder()
                    .bucket(bucketName)
                    .key(metaKey)
                    .build();
    ...
    public FileMetadata getMetadata(String fileId) {
        try {
            ...
            try {
                GetObjectRequest getMetaRequest = GetObjectRequest.builder()
                        .bucket(bucketName)
                        .key(metaKey)
                        .build();
                ...
                log.warn(".meta file not found for fileId: {}, trying to read from S3 object metadata", fileId);
            }
    ...
    private void saveMetadataFile(String fileId, FileMetadata metadata) {
        try {
            ...
            s3Client.putObject(
                    putObjectRequest, RequestBody.fromString(metadataJson, java.nio.charset.StandardCharsets.UTF_8));
