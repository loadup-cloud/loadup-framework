...
-- Table: global_unique (prefix configurable via tablePrefix)
...
CREATE INDEX idx_tenant_id ON global_unique (tenant_id);
...
COMMENT ON COLUMN global_unique.tenant_id IS '租户ID';
...
COMMENT ON COLUMN global_unique.deleted IS '删除标记';

-- 创建触发器以自动更新 updated_at
CREATE OR
REPLACE TRIGGER trg_global_unique_updated_at
    BEFORE
UPDATE ON global_unique
    FOR EACH ROW
