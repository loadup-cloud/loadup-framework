...
**API/binder 模式**（需要多种实现时）：
```
loadup-components-{name}/
├── pom.xml                    # 聚合 POM（packaging: pom）
├── {name}-api/                # SPI 接口 + DTO + 抽象基类
├── {name}-binder-{impl}/      # 每个具体实现一个 binder
└── {name}-test/               # 集成测试
```
- 其他组件依赖时**只引入 `-api`**，不依赖具体 binder
- 集成方运行时按需引入 `-binder-caffeine` 或 `-binder-redis`
- 示例：cache、scheduler、dfs、configcenter、gotone、retrytask
...
└── src/main/java/...
```
- 示例：database（已合并 flyway）、captcha、tracer、signature、authorization、extension

**判断标准**：如果一个组件可能有多于一种后端或协议实现 → API/binder 模式。反之 → 单一 jar。
...
| 位置 | 受众 | 内容 | 维护者 |
|------|------|------|--------|
| `CLAUDE.md`（根目录） | AI 编码助手 | 项目定位、禁止项、构建命令、技术栈 | 开发者 + AI |
...
1. **docs/ 不作为 AI / 开发参考。** docs/ 是 Hugo + Lotusdocs 文档站源码，内容由 `/docs-sync` skill 从模块 README.md 自动同步生成。AI 写代码时以 `CLAUDE.md`、模块 `README.md` 和 `ARCHITECTURE.md` 为准。
