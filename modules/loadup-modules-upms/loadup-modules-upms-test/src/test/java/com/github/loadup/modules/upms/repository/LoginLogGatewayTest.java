...
import com.github.loadup.modules.upms.domain.gateway.LoginLogGateway;
...
class LoginLogGatewayTest extends BaseRepositoryTest {

  @Autowired private LoginLogGateway loginLogGateway;
  ...
  void testSaveLoginLog() {
    // When
    LoginLog saved = loginLogGateway.save(testLog);
  ...
  void testFindByUserId() {
    ...
    LoginLog log2 =
        LoginLog.builder()
            ...
            .loginStatus((short) 1)
            .build();
    loginLogGateway.save(log2);

    // When
    Page<LoginLog> logs = loginLogGateway.findByUserId("1", PageRequest.of(0, 10));
  ...
  void testFindByUsername() {
    ...
    // When
    Page<LoginLog> logs = loginLogGateway.findByUsername("testuser", PageRequest.of(0, 10));
  ...
  void testFindByDateRange() {
    ...
    Page<LoginLog> logs = loginLogGateway.findByDateRange(start, end, PageRequest.of(0, 10));
  ...
  void testFindFailedLogins() {
    ...
    LoginLog failedLog =
        LoginLog.builder()
            ...
            .build();
    loginLogGateway.save(failedLog);
    loginLogGateway.save(testLog); // Successful login

    // When
    LocalDateTime start = LocalDateTime.now().minusHours(1);
    LocalDateTime end = LocalDateTime.now().plusHours(1);
    Page<LoginLog> failedLogs = loginLogGateway.findFailedLogins(start, end, PageRequest.of(0, 10));
  ...
  void testCountLoginAttempts() {
    ...
    LocalDateTime start = LocalDateTime.now().minusHours(1);
    LocalDateTime end = LocalDateTime.now().plusHours(1);
    long count = loginLogGateway.countLoginAttempts("1", start, end);
  ...
  void testCountFailedLoginAttempts() {
    ...
    LoginLog failedLog2 =
        LoginLog.builder()
            ...
            .loginStatus((short) 0)
            .build();
    loginLogGateway.save(failedLog1);
    loginLogGateway.save(failedLog2);
    loginLogGateway.save(testLog); // Successful

    // When
    LocalDateTime start = LocalDateTime.now().minusHours(1);
    LocalDateTime end = LocalDateTime.now().plusHours(1);
    long count = loginLogGateway.countFailedLoginAttempts("1", start, end);
  ...
  void testFindLastSuccessfulLogin() {
    ...
    LoginLog oldLog =
        LoginLog.builder()
            ...
            .loginStatus((short) 1)
            .build();
    loginLogGateway.save(oldLog);
    loginLogGateway.save(testLog); // More recent

    // When
    var lastLogin = loginLogGateway.findLastSuccessfulLogin("1");
  ...
  void testDeleteBeforeDate() {
    ...
    // When
    loginLogGateway.deleteBeforeDate(LocalDateTime.now().minusDays(30));

    // Then
    Page<LoginLog> logs = loginLogGateway.findByUserId("1", PageRequest.of(0, 100));
