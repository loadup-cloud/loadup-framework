...
public class RpcProxyProcessor implements ProxyProcessor {
    ...
    public GatewayResponse proxy(GatewayRequest request, RouteConfig route) throws Exception {
        try {
            ...
            log.error("RPC proxy failed for interface={}", route.getTargetUrl(), e);
            // Rethrow to let the ExceptionAction in the gateway chain handle it uniformly
            throw new RuntimeException("RPC proxy execution failed: " + e.getMessage(), e);
        }
    }
