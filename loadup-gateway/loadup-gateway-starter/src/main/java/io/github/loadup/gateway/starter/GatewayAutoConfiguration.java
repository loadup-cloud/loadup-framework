...
public class GatewayAutoConfiguration {
    ...
    @Bean @ConditionalOnMissingBean public ExceptionFilter exceptionFilter() { return new ExceptionFilter(); }
    @Bean @ConditionalOnMissingBean public ProxyFilter proxyFilter(PluginManager pm) { return new ProxyFilter(pm); }
    @Bean @ConditionalOnMissingBean public ResponseWrapperFilter responseWrapperFilter(GatewayProperties p) { return new ResponseWrapperFilter(p); }
    @Bean @ConditionalOnMissingBean public RateLimitFilter rateLimitFilter(GatewayProperties p) { return new RateLimitFilter(p); }
    @Bean @ConditionalOnMissingBean public CircuitBreakerFilter circuitBreakerFilter(GatewayProperties p) { return new CircuitBreakerFilter(p); }
    @Bean @ConditionalOnMissingBean public SecurityFilter securityFilter(SecurityStrategyManager m) { return new SecurityFilter(m); }
    @Bean @ConditionalOnMissingBean public BodyParserFilter bodyParserFilter() { return new BodyParserFilter(); }

    @Bean
    @ConditionalOnMissingBean
    @ConditionalOnClass(Tracer.class)
    @ConditionalOnProperty(prefix = "loadup.tracer", name = "enabled", havingValue = "true", matchIfMissing = true)
    public TracingFilter tracingFilter(@Autowired(required = false) Tracer t, @Autowired(required = false) TextMapPropagator p) {
        if (t == null || p == null) { log.warn("Tracing disabled"); return null; }
        return new TracingFilter(t, p);
    }

    // --- Security Strategies (direct instantiation) ---

    @Bean @ConditionalOnMissingBean(name = "defaultSecurityStrategy")
    ...
    @Bean @ConditionalOnMissingBean(name = "internalSecurityStrategy")
    ...
    @Bean @ConditionalOnMissingBean(name = "signatureSecurityStrategy")
    ...
    public DefaultGatewayEngine gatewayEngine(
            List<GatewayFilter> filters, RouteResolver routeResolver, RouteStore routeStore,
            ExceptionFilter exceptionFilter,
            @Autowired(required = false) TracingFilter tracingFilter,
            RouteFilter routeFilter, ProxyFilter proxyFilter,
            ResponseWrapperFilter responseWrapperFilter) {
        Map<String, GatewayFilter> registry = filters.stream()
                .collect(Collectors.toMap(GatewayFilter::name, Function.identity(), (a, b) -> a));
        log.info("Gateway filter registry: {}", registry.keySet());
        return new DefaultGatewayEngine(registry, routeResolver, routeStore,
                exceptionFilter, tracingFilter, routeFilter, proxyFilter, responseWrapperFilter);
    }
