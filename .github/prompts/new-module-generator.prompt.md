...
- `{Entity}DO extends BaseDO`（`@Table("{table}")`，不重复定义 id/createdAt/updatedAt/tenantId/deleted）放至
...
- `{Entity}GatewayImpl implements {Entity}Gateway`（用 QueryWrapper 查询，通过 converter 转换）放至
...
生成添加到 `loadup-application/src/main/resources/application.yml` 的路由配置，含 listAll / create / getById / update /
delete 五个路由。
